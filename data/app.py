...
@app.post("/transform/step_3")
def transform_item_step_3(signal_location: Annotated[str | None, Header()] = None):
    

    try:
        signal_location = utilities.normalize_path(signal_location)

        logger.info(signal_location)

        with open(signal_location, 'rb') as f:
            signal_np = np.load(signal_location)
        ...
        for i in range(0,14,1):
            X.append(signal_np[i*22050:(i+1)*22050].astype(np.float32).tolist())
            # X.append(json.dumps(signal_np[i*22050:(i+1)*22050]))


        response_dict = {index: value for index, value in enumerate(X)}


        return {
            "progress": "76%",
            "signal": X,
            "frames": response_dict
        }
